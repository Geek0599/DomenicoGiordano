(() => {
    ...
    function playVideoOnClickPlayBtn() {
        const videosBody = document.querySelectorAll("[data-video-body]");
        if (videosBody.length) videosBody.forEach((videoBody => {
            const video = videoBody ? videoBody.querySelector("video") : null;
            const playBtn = videoBody ? videoBody.querySelector("[data-playicon-btn]") : null;
            if (videoBody && video && playBtn) {
                playBtn.addEventListener("click", (() => playPause(videoBody, video)));
        ...
        function playPause(videoBody, video) {
            if (video.paused) {
                videoBody.classList.remove("_paused");
                video.play();
                setTimeout((() => {
    ...
    function removeVideoCoverOnFullscreen() {
        document.addEventListener("fullscreenchange", (function() {
            if (document.fullscreenElement && document.fullscreenElement.tagName === "VIDEO") document.fullscreenElement.classList.add("_no-cover"); else {
                const allVideos = document.querySelectorAll("video");
                if (allVideos.length) allVideos.forEach((video => {
                    video.classList.remove("_no-cover");
                }));
            }
        }));
    }
